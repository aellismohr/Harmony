...
class CharacterCreationUI:
    ...
    def _load_character_from_json(self, buffer: io.BytesIO) -> bool:
        ...
        try:
            ...
            # Ensure essence traits array has exactly what was saved (no padding with empty strings)
            essence_traits = data["Essence Traits"]
            st.session_state.essence_traits = essence_traits + [""] * (4 - len(essence_traits))

            # --- Create character using bypass method for loaded characters -----------
            success = self._create_character_from_loaded_data(data)
            
            if success:
                st.session_state.character_created = True
                return True
            else:
                st.error("Failed to create character from loaded data")
                return False
    ...
    def _create_character_from_loaded_data(self, data):
        """Create a character from loaded JSON data, bypassing validation that might fail for old saves"""
        try:
            # Extract genetics data
            genetics_data = data["Genetics"]
            father_markers = genetics_data["Father Markers"]
            mother_markers = genetics_data["Mother Markers"] 
            planet_markers = genetics_data["Planet Markers"]
            ...
            # Get selected traits
            physical_trait = data["Physical Trait"]
            mental_trait = data["Mental Trait"]
            social_trait = data["Social Trait"]
            livelihood_trait = data["Livelihood Trait"]
            travel_trait = data["Travel Trait"]
            essence_traits_choice = data["Essence Traits"]
            
            # Create player name
            player_name = data["Name"]
            
            # Create the character object directly without validation
            if 'Genetics' in globals() and 'Player' in globals():
                try:
                    ...
                    # Restore skill-set progress if available
                    if "SkillXP" in data:
                        xp_table = data["SkillXP"]
                    ...
                    st.error(f"Error creating character object from loaded data: {str(e)}")
                    import traceback
                    st.error(traceback.format_exc())
                    return False
            else:
                st.error("Player or Genetics classes not available")
                return False
                
        except Exception as e:
            st.error(f"Error processing loaded character data: {str(e)}")
            import traceback
            st.error(traceback.format_exc())
            return False
    ...
    def old_load_character_from_json(self, buffer: io.BytesIO) -> bool:
        """Parse a save-file and push its fields into session_state."""
        try:
            data = json.load(buffer)
